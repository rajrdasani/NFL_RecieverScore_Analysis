...
```{r}
wrscore_21_22 %>%
  ggplot(aes(x = X2021_Overall, y = X2022_Overall)) + 
...
Overall, WR Scores stay mostly consistent , but there are definitely players who's score change pretty significantly year to year. My hypothesis for this is a couple different factors:
- previous year success (teams start to double this player more often)
- team change (either the player himself or a change on the team)
- injury in the current year (catch score doesnt have chance to stabilize)
...
wrscore_alldelta %>%
  ...
  ggplot(aes(x = reorder(Player, -overall_delta), y = overall_delta, fill = color, label = overall_delta)) + 
    ...
    geom_label() +
    theme_minimal() +
    labs(title = "Top 15 Biggest Delta in Overall WR Score between 2017 to 2022",
         x = "Player", 
